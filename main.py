...
# 1. Убираем stateless_http=True, так как Uvicorn (SSE) работает с постоянным подключением
...
DISTRICTS = {
    "Сарыарка": {"avg_dl": 92.4, "avg_ul": 92.2, "avg_ping": 41.9, "bad_pct": 32.1, "pts": 218,
                 "center": [51.175, 71.42], "severity": "HIGH"},
    "Алматы": {"avg_dl": 83.3, "avg_ul": 73.7, "avg_ping": 36.5, "bad_pct": 27.4, "pts": 398, "center": [51.115, 71.39],
               "severity": "HIGH"},
    "Байконур": {"avg_dl": 119.0, "avg_ul": 96.4, "avg_ping": 26.7, "bad_pct": 20.1, "pts": 199,
                 "center": [51.085, 71.44], "severity": "MEDIUM"},
    "Нура": {"avg_dl": 135.5, "avg_ul": 125.0, "avg_ping": 35.8, "bad_pct": 16.4, "pts": 782, "center": [51.19, 71.47],
             "severity": "MEDIUM"},
    "Есиль": {"avg_dl": 160.1, "avg_ul": 133.7, "avg_ping": 30.7, "bad_pct": 8.5, "pts": 656, "center": [51.155, 71.47],
              "severity": "LOW"},
}
...
def analyze_port_failure(port_id: int) -> str:
    ...
    try:
        ...
        return json.dumps({
            "status": "error", 
            "message": "GIS-сервер (techa.etquickprice.kz) не ответил вовремя. Пожалуйста, попробуйте еще раз чуть позже."
        }, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"status": "error", "message": f"Ошибка запроса: {str(e)}"}, ensure_ascii=False)
...
def analyze_multiple_port_failures(port_ids: list[int]) -> str:
    ...
    for port_id in port_ids[:5]:
        try:
            ...
            results.append({"port_id": port_id, "status": "error", "message": "Тайм-аут запроса к GIS"})
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print("=" * 55)
    print(f"  Запуск Astana Network Analyzer на порту {port} (Uvicorn / SSE)")
    print("=" * 55)

    # FastMCP в режиме SSE автоматически использует Uvicorn как ASGI-сервер
